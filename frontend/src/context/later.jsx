...
const Dashboard = () => {
  ...
  if (loadingTeams || loadingDashboard) return <div>Loading dashboard...</div>;
  ...
  return (
    <div className="min-h-screen px-6 py-8 bg-background-dark text-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {teams.map(team => (
              <div key={team.id} className="bg-card-dark p-6 rounded-2xl border border-accent-dark">
                <h3 className="text-lg font-bold mb-4"><Link to={`/manage/${team.id}`}>{team.name}</Link></h3>
                {team.members.map(member => {
                  const isMe = member.player.user.id === user.id;
                  return (
                    <div key={member.id} className={`p-3 rounded-xl border mb-2 ${isMe ? "border-primary bg-primary/10" : "border-slate-700 bg-accent-dark/40"}`}>
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="font-bold">{member.player.user.username}{isMe && <span className="text-primary ml-2">(You)</span>}</p>
                          <p className="text-xs text-slate-400">Role: {member.role}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold">{member.player.mmr} MMR</p>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Applications */}
        <div>
          <h2 className="text-xl font-bold mb-4">Applications Sent</h2>
        ...
        <div>
          <h2 className="text-xl font-bold mb-4">Applications Received</h2>
        ...
        {/* Invites */}
        <div>
          <h2 className="text-xl font-bold mb-4">Invites Sent</h2>
        ...
        <div>
          <h2 className="text-xl font-bold mb-4">Invites Received</h2>
