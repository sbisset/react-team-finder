        ...
        {teams.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {teams.map((team) => (
              <div
                key={team.id}
                className="bg-card-dark p-6 rounded-2xl border border-accent-dark"
              >
                <h3 className="text-lg font-bold mb-4">
                  {team.owner_id === user.id ? (
                    <Link to={`/manage/${team.id}`} className="text-primary">
                      {team.name}
                    </Link>
                  ) : (
                    <span>{team.name}</span>
                    ...
                    <div
                      ...
                      className={`p-3 rounded-xl border mb-2 ${
                        isMe
                          ...
                          : "border-slate-700 bg-accent-dark/40"
                      }`}
                    >
          ...
          <div className="mb-6">
            ...
            {dashboardData.sent_applications.length === 0 ? (
              ...
              dashboardData.sent_applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-card-dark p-3 border border-slate-700 rounded-xl mb-2"
                >
                  <p>
                    <strong>Team:</strong> {app.team} |{" "}
                  ...
                  <button
                    onClick={() =>
                      setSelectedRequest({ id: app.id, type: "sent_apps" })
                    }
